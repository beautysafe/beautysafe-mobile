...
export default function UnavailableProductScreen() {
  ...
  /**
   * EAN received from the product search / scanner.
   */
  const routeEan =
    typeof ean === "string" &&
    ean.trim()
  ...
  /**
   * Keep the current EAN separately from route params.
   *
   * This allows "Ajouter un autre produit" to create
   * a completely fresh submission without reusing
   * the previous product's EAN.
   */
  const [
    currentEan,
    setCurrentEan,
   ...
   * IMPORTANT FIX:
   *
   * Expo Router may preserve a route/component in
   * the navigation stack.
   *
   * Therefore, when this screen becomes active again,
   * always reset the old "Merci" state and prepare
   * a fresh form.
   *
   * Example:
   *
  ...
  /**
   * Add selected gallery/camera assets.
   */
  const addAssets = (
    assets:
      ImagePicker.ImagePickerAsset[],
  ...
  /**
   * Select one or several images from gallery.
   */
  const selectFromGallery =
    async () => {
      if (
        ...
        Alert.alert(
          "Maximum atteint",
          "Vous pouvez envoyer jusqu’à 10 photos.",
        );
  ...
  const handleSubmit =
    async () => {
      ...
      if (
        ...
        setErrorMessage(
          "Ajoutez au moins une photo avant l’envoi.",
        );
  ...
  /**
   * User wants to submit another unavailable product
